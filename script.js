...
function saveData() {
  try {
    localStorage.setItem('my_text_app_data_v2', JSON.stringify(appData));
  } catch(e) {
...
// 確定データ読み込み
function loadData() {
  const rawData = localStorage.getItem('my_text_app_data_v2');
  ...
  if (!Array.isArray(appData.folders)) appData.folders = ['重要問', '要復習'];
...
function renderSubjectFolders() {
  ...
  if (subjects.length === 0) {
    container.innerHTML = `<p style="grid-column:1/-1; color:#8e8e93; text-align:center; padding: 10px 0;">問題がまだありません。</p>`;
    return;
  }
...
function openSubFields(subjectName) {
  ...
  container.innerHTML = fields.map(field => {
    ...
    return `
      <div class="book-card" style="border-left: 4px solid #4f46e5;" onclick="filterBySubField('${subjectName}', '${field}')">
...
function handleSaveQuestion(e) {
  ...
  const questionObj = {
    ...
    lastStatus: existing ? existing.lastStatus : null, // 「解けた」「イマイチ」「不正解」
...
function renderQuestions(listToRender = appData.questions, containerId = 'questions-list') {
...
function filterReviewList(type, btnEl) {
  ...
  renderQuestions(list, 'review-list');
}
...
function recordAssessment(qId, status) {
  ...
  closeReviewModal();
  renderQuestions();
  showToast(`記録しました（${status}）- 次回復習: ${q.nextReviewDate}`);
}
